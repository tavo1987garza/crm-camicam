...
# 📌 Endpoint para recibir mensajes desde WhatsApp (creación automática de lead)
...
def recibir_mensaje():
    ...
    if conn:
        try:
            ...
            if not lead:
                # 🔹 Si no tiene nombre, asignar un nombre por defecto (últimos 10 dígitos)
                nombre_por_defecto = f"{remitente[-10:]}"
                cursor.execute("""
            ...
            cursor.execute("""
                INSERT INTO mensajes (plataforma, remitente, mensaje, estado)
                VALUES (%s, %s, %s, 'Nuevo')
...
def obtener_leads():
    ...
    try:
        ...
        cursor.execute("""
            SELECT l.*, 
                   COALESCE(
                     (SELECT mensaje FROM mensajes WHERE remitente = l.telefono ORDER BY fecha DESC LIMIT 1),
...
def crear_lead():
    ...
    try:
        ...
        if lead_id:
            ...
            socketio.emit("nuevo_lead", nuevo_lead)  # Notificar en tiempo real
...
def eliminar_lead():
    ...
    try:
        datos = request.json
        lead_id = datos.get("id")
        telefono = datos.get("telefono")  # Para borrar también sus mensajes
...
# 📌 Endpoint para editar un lead
@app.route('/editar_lead', methods=['POST'])
def editar_lead():
    conn = None
    data = request.get_json()
    print("📌 Datos recibidos en /editar_lead:", data)
    lead_id = data.get("id")
...
# 📌 Mostrar los mensajes de cada chat y detalles del lead
